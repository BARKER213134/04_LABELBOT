...
class TelegramConversationHandler:
    ...
    async def fallback_text_handler(self, update: Update, context) -> int:
        """Fallback handler for text messages when ConversationHandler state is lost"""
        user_id = str(update.effective_user.id)
        ...
        if not state_doc:
            # No saved state - user is not in a conversation
            logger.warning(f"[FALLBACK] No state for user {user_id}, ignoring message")
            return ConversationHandler.END
        
        saved_state = state_doc.get('state')
        saved_data = state_doc.get('data', {})
        
        # Restore data to memory
        self.user_data[user_id] = saved_data
        
        logger.warning(f"[FALLBACK] Restored state {saved_state} for user {user_id}")
        ...
        # Unknown state - end conversation
        logger.warning(f"[FALLBACK] Unknown state {saved_state} for user {user_id}")
        return ConversationHandler.END
